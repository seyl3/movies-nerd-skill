...
3. Resolve the exact title, year, media type, runtime, and authoritative IDs before searching. Create one private v2 manifest under `<LIBRARY>/.movies-nerd/jobs` with `scripts/job_manifest.py` and update it after each material transition. Resume from this manifest rather than repeating search, confirmation, or work.
4. Search read-only and API-first. Run `scripts/search_releases.py` with its five-second default budget and `--imdb-id` when known; it queries YTS, Knaben, APIBay, and Magnetz concurrently and uses configured qBittorrent/Torznab providers when needed. Prefer validated YTS direct `.torrent` metadata, but fall back to a tracker-aware magnet for the same hash. Rank reported peers only as estimates and exclude recently dead hashes. Follow [references/search.md](references/search.md).
...
10. Record confirmation with `scripts/job_manifest.py transition --event confirmed`, then run the single deterministic controller: `scripts/acquire.py --job ... --commit`. Do not manually orchestrate separate racing and monitoring scripts. It performs a silent qBittorrent readiness check, opens the app, adds at most three candidates, validates direct `.torrent` or magnet metadata, applies Gate 1, and briefly measures real byte growth. As soon as two candidates are clearly healthy—and always within the bounded comparison window—it keeps the best one and permanently removes every duplicate candidate and its partial data. Never carry two downloads of the same title beyond comparison, and never retain a warm standby in qBittorrent. It tries a second wave only when needed.
11. Download only into the selected hidden staging directory. Send at most: **“Into the Abyss (2011) is downloading. I’ll let you know when it’s ready.”** Do not expose codec, inspection, exclusion, staging, client, candidate, or failover details. Recheck the single-transfer invariant whenever a job resumes. The controller replaces a candidate after roughly 30 seconds without metadata, 60 seconds without byte growth, or 90 seconds of persistently unusable speed when another confirmed candidate or one refresh remains. It probes the next untried candidate inside the confirmed envelope, chooses one winner again, and removes every comparison copy. It never asks again. When it emits `enrichment: start-now`, immediately run `finalization_queue.py start-all` and prepare metadata, artwork, English/French subtitles, destination paths, direct film links, and recommendations concurrently according to [references/finalization.md](references/finalization.md).
...
17. Apply the hard cleanup invariant before the ready message. After the final folder has verified media, NFO, and artwork, remove every exact Movies Nerd-owned active, comparison loser, legacy standby, and failed qBittorrent entry through `qbittorrent_api.py remove --commit`; never touch an untagged or non-job path. Then run `scripts/finish_staging.py --job ... --commit` with each exact transfer, clean, sidecar, and partial-data path. It must verify no matching qBittorrent entry, manifest, lock, temporary `.torrent`, job trash, staging path, empty `.incoming/Movies Nerd`, or AppleDouble sidecar remains. The bounded provider-health cache is the only persistent cross-job state. If cleanup cannot be proven, keep the manifest and continue the job; do not report ready.
...
- `scripts/finalization_queue.py`: Persist the independent metadata, artwork, subtitle, film-link, recommendation, and destination work started during transfer.
