...
13. Reuse the saved Gate 2 probe for subtitle coverage, track labels, chapters, and the remux decision. Reject it if the media snapshot changed. If an existing MKV already complies, keep it unchanged. Otherwise use `scripts/remux_mkv.py --probe-json` once for verified stream copy and probe only the newly written output for comparison. Do not re-encode merely to change containers. Do not add media hashes or checksum manifests to this pipeline.
