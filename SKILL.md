...
10. Record confirmation with `scripts/job_manifest.py transition --event confirmed`, then run the single deterministic controller: `scripts/acquire.py --job ... --commit`. Do not manually orchestrate separate racing and monitoring scripts. It performs a silent qBittorrent readiness check, opens the app, adds at most three candidates, validates direct `.torrent` or magnet metadata, applies Gate 1, measures real byte growth under bounded probes, starts the fastest, and keeps one stopped warm standby. It tries a second wave only when needed and removes other candidates.
11. Download only into the selected hidden staging directory. Send at most: **“Into the Abyss (2011) is downloading. I’ll let you know when it’s ready.”** Do not expose codec, inspection, exclusion, staging, client, candidate, or failover details. The controller replaces a candidate after roughly 30 seconds without metadata, 60 seconds without byte growth, or 90 seconds of persistently unusable speed when a healthier standby exists. It activates the warm standby first, then probes the remaining envelope, then performs one fresh API search inside that envelope. It never asks again. When it emits `enrichment: start-now`, immediately run `finalization_queue.py start-all` and prepare metadata, artwork, English/French subtitles, destination paths, direct film links, and recommendations concurrently according to [references/finalization.md](references/finalization.md).
...
- `scripts/acquire.py`: Run one confirmed job through qBittorrent preflight, bounded live probes, warm standby, monitoring, failover, one in-envelope refresh, crash resume, and transfer completion.
- `scripts/race_candidates.py`: Measure up to three confirmed candidates at a time using real byte growth and keep one stopped warm standby.
...
- Prefer source diversity, validated YTS direct metadata, then distinct compatible hashes. Keep reported seeders as estimates. Let the live probe decide, retain one warm standby, silently exhaust two confirmed waves, and perform one in-envelope API refresh before returning to the user.
