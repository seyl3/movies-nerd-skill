...
- Resume from the manifest. `scripts/run_job.py` holds one atomic movie-job lock through download, prepared-artifact waiting, finalization, and cleanup; `scripts/acquire.py` provides the same acquisition resume behavior for series. Reconnect to the existing active torrent instead of repeating search or confirmation.
